...
for p in iss:
        ...
        with open("TotalParameterMatrix"+p+"Test.csv") as f:
            ...
            with open(os.getcwd()+"\\"+p+"\\DNeuralNetwork\\resultsTest.csv",'w') as f:
                    writer = csv.writer(f)
                    ...
                    for i in [0,1,2,3,4]:
                        ...
                        if(p=='Leon3'):
                                dictt={1: 150,2:150,3:500,4:1000,5:1000 }
                        elif(p=='Atmega328p'):
                                dictt={1: 150,2:150,3:250,4:500,5:1000 }
                        elif(p=="Armv4t"):
                                dictt={1: 150,2:150,3:1000,4:1000,5:1000 }
                        elif(p=="Armv6-M"):
                                dictt={1: 150,2:150,3:1000,4:500,5:1000 }
                #single target
                        ...
                        filename = os.getcwd()+'\\'+p+'\\DNeuralNetwork\\Epoch'+str(dictt[i+1])+'_'+dataframeFeatureMin.columns[count+i]+'.h5'
                        training_t=time.time()
                        reg=tf.keras.models.load_model(filename)
                        ...
                        plt.plot(500, 500)
                        plt.savefig(os.getcwd()+'\\'+p+'\DNeuralNetwork\epoch'+str(dictt[i+1])+dataframeFeatureMin.columns[count+i]+'TestResidual.png')
                        plt.close()
                        ...
                        writer.writerow([dataframeFeatureMin.columns[count+i],str(dictt[i+1]),MSE,NRMSE,rmspe,MAE,R2,training_t,MAPE])