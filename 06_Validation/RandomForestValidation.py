...
# load dataset
        ...
        with open(os.getcwd()+"\\"+p+"\\RandomForest\\resultsValidation.csv",'w') as f:
            writer = csv.writer(f)
        ...
        #single target
            for i in [0,1,2,3,4]:
                ...
                for depth in [8,10,12,16]:
                    testing_t=time.time()
                    filename = os.getcwd()+'\\'+p+'\RandomForest\depth'+str(depth)+'_'+firstline[36+i]+'.sav'
                    reg=pickle.load(open(filename, 'rb'))
                    pp=reg.predict(x_test)
                    testing_t=time.time()-testing_t
                    ...
                    try:
                        rmspe = (np.sqrt(np.mean(np.square(np.divide((y_test - pp), y_test))))) * 100
                    except:
                        rmspe=0
