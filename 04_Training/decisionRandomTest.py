...
# load dataset
        ...
        with open(os.getcwd()+"\\"+p+"\\REG_TREE\\resultsTest.csv",'w') as f:
            ...
            for i in [0,1,2,3,4]:
                ...
                filename = os.getcwd()+'\\'+p+'\REG_TREE\depth'+str(dictt[i+1])+'_'+firstline[37+i]+'.sav'
                ...
                plt.plot(500, 500)
                plt.savefig(os.getcwd()+'\\'+p+'\\REG_TREE\depth'+str(str(dictt[i+1]))+'_Single_'+firstline[37+i]+'TestResidual.png')
                plt.close()
                ...
                writer.writerow(['Single:'+firstline[37+i],str(dictt[i+1]),MSE,NRMSE,rmspe,MAE,R2,training_t,MAPE])
        ...
        with open(os.getcwd()+"\\"+p+"\\RandomForest\\resultsTest.csv",'w') as f:
            ...
            for i in [0,1,2,3,4]:
                x_train = np.concatenate((dataframe.to_numpy()[:,8:37],dataframe.to_numpy()[:,42:len(dataframe.to_numpy()[0,:])]), axis=1)
                filename = os.getcwd()+'\\'+p+'\RandomForest\depth'+str(dictt[i+1])+'_'+firstline[37+i]+'.sav'
                y_train=dataframe.to_numpy()[:,(37+i)]
                reg = pickle.load(open(filename, 'rb'))
                training_t=time.time()
                reg.fit(x_train, y_train)
                ...
                plt.ylabel('Predictions')
                plt.plot(500, 500)
                plt.savefig(os.getcwd()+'\\'+p+'\\RandomForest\depth'+str(str(dictt[i+1]))+'_Single_'+firstline[37+i]+'TestResidual.png')
                plt.close()
