...
dict={1: 'Assembly',2:'Clock',3:'Text',4:'Data',5:'Bss'}
for i in iss:
    for x in range(0,5):
        df = pd.read_csv('TotalParameterMatrix'+i+'.csv', sep=';')
        del df['Unnamed: 0']

        listHeader = list(df.columns.values)
        y = df[listHeader[35+x]].astype(float)
        ...
        file_name = 'allCLFscore'+dict[x+1]+i+'.csv'
        ...
        plt.xscale('log')
        plt.axvline(x=0.001)
        plt.savefig('allCLFscore'+dict[x+1]+i+'.png')
